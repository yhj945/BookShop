...
<div class="x1" id="code"><jsp:include page="bookhouse.jsp"/></div>
<center>
	<table width=80% border=0 cellpadding="15" cellspacing="0" style="text-align:center;">
		<tr height=50px bgcolor="00ccff">
			...
			<a href="<%=request.getContextPath()%>/Project/jsp/buybook1.jsp" style="color:#FFF;">科学类&nbsp;</a>
			</div></td>
			<td name=tdcolor class="t1" width="14%"><div class="h">
			<a href="<%=request.getContextPath()%>/Project/jsp/buybook2.jsp">言情类&nbsp;</a>
			</div></td>
			<td name=tdcolor class="t1" width="14%"><div class="h">
			...
			<form action="<%=request.getContextPath()%>/searchGoods?book=1" method="post" name="searchbook1" >
			<select name="select1" id="selectid1" style="height:25px;">
				<option value="g_Price">价格</option>
				<option value="g_Name">书名</option>
				<input type="text" name="input1" size="25" style="height:19px;font-size:14px"/>
...
String sql = null;
if(select1 != null && input1.length() > 0){
	if("g_Name".equals(select1)){
		sql="select * from Goods where t_ID = '01' and g_Name like '%"+input1+"%'";
	}else if("g_Price".equals(select1)){
		sql="select top 5 * FROM Goods where t_ID='01' order by(ABS(g_Price-"+input1+"))";
	}
} else {
	sql="select * from Goods where t_ID = '01'";
}
...
%>
   		<tr height=250px bgcolor="ebf7f7" name="changecolor">
	    	...
	    	<td class="t2">
	   		...
	   		<td class="t2">
	   			<input id="btn" class="btn" type="submit" value="加入购物车" onclick="x1()" style="font-size:20px;"/>
	   		</td>
	   		</form>
	   		</td>
		</tr>                  
