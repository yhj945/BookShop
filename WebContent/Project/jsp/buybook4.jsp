...
<center>
	<table width=80% border=0  cellpadding="15" cellspacing="0" style="text-align:center;">
		<tr height=50px bgcolor="00ccff">
			...
			<a href="<%=request.getContextPath()%>/Project/jsp/buybook4.jsp" style="color:#FFF;">玄幻类&nbsp;</a>
			</div></td>
			<td class="t1" width="30%">			
			<form action="<%=request.getContextPath()%>/searchGoods?book=4" method="post" name="searchbook4" >
			<select name="select4" id="selectid4" style="height:25px">
				<option value="g_Price">价格</option>
				<option value="g_Name">书名</option>
				<input type="text" name="input4" size="25" style="height:19px;font-size:14px"/>
...
String select4 =(String)session.getAttribute("select4");
String input4 =(String)session.getAttribute("input4");
String sql = null;
if(select4 != null && input4.length() > 0){
	if("g_Name".equals(select4)){
		sql="select * from Goods where t_ID = '04' and g_Name like '"+'%'+input4+'%'+"'";
	}else{
		sql="select top 5 * FROM Goods where t_ID='04' order by(ABS(g_Price-"+input4+"))";
	}
}else {
	sql="select * from Goods where t_ID = '04'";
}
