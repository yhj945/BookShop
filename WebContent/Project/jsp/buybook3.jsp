...
<center>
	<table width=80% border=0  cellpadding="15" cellspacing="0" style="text-align:center;">
		<tr height=50px bgcolor="00ccff">
			...
			<a href="<%=request.getContextPath()%>/Project/jsp/buybook3.jsp" style="color:#FFF;">历史类&nbsp;</a>
			</div></td>
			<td name=tdcolor class="t1" width="14%"><div class="h">
			<a href="<%=request.getContextPath()%>/Project/jsp/buybook4.jsp">玄幻类&nbsp;</a>
			</div></td>
			<td class="t1" width="30%">			
			<form action="<%=request.getContextPath()%>/searchGoods?book=3" method="post" name="searchbook3" >
			<select name="select3" id="selectid3" style="height:25px">
				<option value="g_Price">价格</option>
				<option value="g_Name">书名</option>
				<input type="text" name="input3" size="25" style="height:19px;font-size:14px"/>
...
String input3 =(String)session.getAttribute("input3");
String sql = null;
if(select3 != null && input3.length() > 0){
	if("g_Name".equals(select3)){
		sql="select * from Goods where t_ID = '03' and g_Name like '"+'%'+input3+'%'+"'";
	}else{
		sql="select top 5 * FROM Goods where t_ID='03' order by(ABS(g_Price-"+input3+"))";
	}
}else {
	sql="select * from Goods where t_ID = '03'";
}
